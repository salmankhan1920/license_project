...
function setText() {
    var element = document.getElementsByClassName("usermenu__info-name")[0]; // Get the element to edit
    if (isMobile()) {
        element.innerHTML = "LIVE"; // Text for mobile
    } else {
        element.innerHTML = "LIVE ACCOUNT"; // Text for desktop
    }
}

setTimeout(setText, 100); // Delay execution by 100 milliseconds



var tradeButton = document.querySelector('a[data-text="Trade"]');
tradeButton.classList.add('active');
var tradeButton = document.querySelector('a[data-text="Trade"]');
// Add click event listener to the document
document.addEventListener('click', function(event) {
    // Add the "active" class to the "Trade" button
    tradeButton.classList.add('active');
});

var elementToRemove = document.querySelector('.header__banner');
